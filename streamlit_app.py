...
# ============================================================
#            ENHANCED THEME CSS WITH BETTER SERVER NAME
# ============================================================
normal_css = """
<style>
    ...
    /* Header styling with enhanced server name */
    .main-header {
    ...
    @keyframes shimmer {
        ...
        50% { transform: translate(30%, 30%) rotate(180deg); opacity: 0.2; }
        100% { transform: translate(-30%, -30%) rotate(360deg); opacity: 0.5; }
    }
    ...
    /* Glowing server name in sidebar */
    .server-glow {
        background: linear-gradient(135deg, #667eea, #764ba2);
...
def admin_panel():
    ...
    <div class="main-header">
        ...
        <h1>✦ ADMIN PANEL ✦</h1>
        <div class="server-badge">⚡ KEY APPROVAL MANAGEMENT ⚡</div>
        <p>🎯 E2E By Suraj Oberoy - Server Control Center 🎯</p>
    </div>
...
def approval_request_page(user_key, username):
    ...
    <div class="main-header">
        ...
        <div class="server-badge">✨ ONE MONTH - 0 RS PAID ✨</div>
        <p>🔐 E2E By Suraj Oberoy - Secure Access Required 🔐</p>
    </div>
    """, unsafe_allow_html=True)
...
def login_page():
    ...
    <div class="main-header">
        <div class="prince-logo"><span>👑</span></div>
        <h1>✦ E2E BY SURAJ OBEROY ✦</h1>
        <div class="server-badge">🚀 PREMIUM AUTOMATION SERVER 🚀</div>
...
def main_app():
    ...
    <div class="main-header">
        <div class="prince-logo"><span>👑</span></div>
        <h1>✦ E2E BY SURAJ OBEROY ✦</h1>
        <div class="server-badge">⚡ OFFLINE AUTOMATION SERVER ⚡</div>
        <p>"seven billion smiles in this world but yours is my favorite___✨"</p>
    </div>
    """, unsafe_allow_html=True)
    ...
    <div class="sidebar-header">
        ...
        <div style="font-weight: 800; background: linear-gradient(135deg, #667eea, #764ba2); -webkit-background-clip: text; -webkit-text-fill-color: transparent;">
            E2E BY SURAJ
        </div>
        <div style="font-size: 0.7rem; color: #888;">PREMIUM SERVER</div>
    </div>
    """, unsafe_allow_html=True)
    ...
    if user_config:
        ...
        with tab1:
            ...
            # ========== NEW: Message File Upload Section ==========
            st.markdown("---")
            st.markdown("### 📁 Upload Messages File")
            st.caption("Upload a .txt file with one message per line (max 100 messages)")
            
            uploaded_file = st.file_uploader("Choose a text file", type=['txt'], key="msg_file_uploader")
            ...
            if uploaded_file is not None:
                try:
                    ...
                    messages_from_file = [line.strip() for line in lines if line.strip()]
                    
                    if messages_from_file:
                        st.markdown(f'<div class="uploaded-file">📄 Loaded <strong>{len(messages_from_file)}</strong> messages from <strong>{uploaded_file.name}</strong></div>', unsafe_allow_html=True)
                        
                        # Preview first 5 messages
                        with st.expander("📋 Preview Uploaded Messages"):
                            for i, msg in enumerate(messages_from_file[:10], 1):
                                st.markdown(f"**{i}.** {msg}")
                            if len(messages_from_file) > 10:
                                st.info(f"... and {len(messages_from_file) - 10} more messages")
                        
                        # Option to append or replace
                        col1, col2 = st.columns(2)
                        with col1:
                            if st.button("📝 Replace Current Messages", use_container_width=True):
                                # Replace existing messages
                                new_messages = "\n".join(messages_from_file)
                                update_user_config(
                                ...
                                st.success(f"✅ Replaced with {len(messages_from_file)} messages from file!")
                                time.sleep(1)
                                st.rerun()
                        
                        with col2:
                            if st.button("➕ Append to Current Messages", use_container_width=True):
                                # Append to existing messages
                                current_msgs = user_config['messages']
                                if current_msgs:
                                    new_messages = current_msgs + "\n" + "\n".join(messages_from_file)
                                else:
                                    new_messages = "\n".join(messages_from_file)
                                update_user_config(
                                    st.session_state.user_id,
                                ...
                                st.success(f"✅ Appended {len(messages_from_file)} messages!")
                                time.sleep(1)
                                st.rerun()
                    else:
                        st.warning("⚠️ File is empty or contains no valid messages!")
                except Exception as e:
                    st.error(f"❌ Error reading file: {str(e)}")
            
            st.markdown("---")
            st.markdown("### ✍️ Messages (One per line)")
            
            messages = st.text_area("Your Messages",
                                   value=user_config['messages'],
                                   placeholder="Enter each message on a new line\nExample:\nHello!\nHow are you?\nNice to meet you!",
                                   height=200,
                                   help="Enter each message on a new line. These will be sent in rotation.")
            
            # Show message count
            msg_lines = [msg.strip() for msg in messages.split('\n') if msg.strip()]
            st.caption(f"📊 Total messages: **{len(msg_lines)}**")
        ...
        with tab2:
            ...
            with col1:
                if st.button("▶️ Start Automation", disabled=st.session_state.automation_state.running, use_container_width=True):
                    ...
                    if user_config and user_config['chat_id']:
                        ...
                        if not user_config.get('messages', '').strip():
                            st.warning("⚠️ No messages found! Please add messages in Configuration tab first.")
                        else:
            ...
            # Display current messages that will be sent
            if user_config and user_config.get('messages', '').strip():
                with st.expander("📋 Current Message Queue"):
                    msg_list = [msg.strip() for msg in user_config['messages'].split('\n') if msg.strip()]
                    for i, msg in enumerate(msg_list, 1):
                        st.markdown(f"**{i}.** {msg}")
            else:
                st.info("ℹ️ No messages configured. Go to Configuration tab to add messages.")
...
st.markdown("""
<div class="footer">
    Made with ❤️ by <span style="color: #667eea; font-weight: bold;">SURAJ OBEROY</span> | <span style="color: #764ba2;">E2E PREMIUM SERVER</span> | © 2026
</div>
""", unsafe_allow_html=True)
