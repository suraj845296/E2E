...
def run_automation_with_notification(user_config, username, automation_state, user_id):
    ...
    if cookies:
        log_message(f"🍪 Cookies loaded ({len(cookies)} chars). Using authenticated session.", automation_state)
    else:
        log_message(f"⚠️ No cookies set. Some features may not work.", automation_state)
...
def user_main_app():
    ...
    if user_config:
        ...
        tab1, tab2, tab3, tab4 = st.tabs(["⚙️ Configuration", "🍪 Cookies Settings", "🚀 Automation", "🤖 Auto-Reply"])
